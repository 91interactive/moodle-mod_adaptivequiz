...
$PAGE->set_url('/mod/catadaptivequiz/delattempt.php', ['attempt' => $attempt->id]);
$PAGE->set_title(format_string($adaptivequiz->name));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->set_context($context);

$returnurl = new moodle_url('/mod/catadaptivequiz/viewattemptreport.php', ['cmid' => $cm->id, 'userid' => $user->id]);
...
if ($confirm) {
    ...
    $DB->delete_records('catadaptivequiz_attempt', ['id' => $attempt->id]);

    adaptivequiz_update_grades($adaptivequiz, $user->id);

    $message = get_string('attemptdeleted', 'catadaptivequiz', $a);
    redirect($returnurl, $message, 4);
}

$message = get_string('confirmdeleteattempt', 'catadaptivequiz', $a);

$confirm = new moodle_url('/mod/catadaptivequiz/delattempt.php', ['attempt' => $attempt->id, 'confirm' => 1]);
echo $OUTPUT->header();
